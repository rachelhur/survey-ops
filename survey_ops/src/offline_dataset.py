...
class OfflineDECamDataset(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        states, next_states = self._construct_states(df=df)
    ...
    def _get_point_features(self, df):
        ...
        feats_in_df = ['ra', 'dec', 'az', 'zd', 'airmass', 'ha']
        pointing_states[:, :len(feats_in_df)] = df[feats_in_df].to_numpy()
        pointing_states[:, self.featname2idx['el']] = 90 - pointing_states[:, self.featname2idx['el']]
        ...
        for idx, time in enumerate(timestamps):
            ...
            pointing_states[idx, self.featname2idx['sun_ra']] = sun_ra
            pointing_states[idx, self.featname2idx['sun_dec']] = sun_dec
            pointing_states[idx, self.featname2idx['moon_ra']] = moon_ra
            pointing_states[idx, self.featname2idx['moon_dec']] = moon_dec
            pointing_states[idx, self.featname2idx['sun_az']], pointing_states[idx, self.featname2idx['sun_el']] = ephemerides.equatorial_to_topographic(ra=sun_ra, dec=sun_dec, time=time)
            pointing_states[idx, self.featname2idx['moon_az']], pointing_states[idx, self.featname2idx['moon_el']] = ephemerides.equatorial_to_topographic(ra=moon_ra, dec=moon_dec, time=time)
        ...
        for dt, g_df in groups:
            ...
            if t_end == t_start: # only one observation that night
                pointing_states[indices, self.featname2idx['time_fraction_since_start']] = 0
                pointing_states[indices, self.featname2idx['time_seconds_till_end']] = 0
            else:
                pointing_states[indices, self.featname2idx['time_fraction_since_start']] = (times - t_start) / t_tot
                pointing_states[indices, self.featname2idx['time_seconds_till_end']] = t_tot - (times - t_start)

        # # Normalize states
        # periodic_names = set(['ra', 'az'])
        # for feat_name, feature_array in zip(self.pointing_feature_names, pointing_states.T):
        #     if 
    ...
    def _construct_actions(self, next_states, bin_space='radec', binning_method='healpix', num_bins_1d=None):
        ...
        if binning_method == 'uniform_grid' and bin_space == 'azel':
            ...
            ra_idx = self.featname2idx['ra']
            dec_idx = self.featname2idx['dec']
